...
pipeline {
    ...
    environment {
        ...
        PYTHON_IMAGE_VERSION = "python:3.11-slim"
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        if ("${list[i]}" == "'UnitTests'" && env.ACTION == 'DEPLOY') {
                            stage('Unit Tests') {
                                print(list[i])
                                // stage details here
                                sh 'docker run -v "$WORKSPACE":/app -w /app $PYTHON_IMAGE_VERSION /bin/sh unitTest.sh'
                            }
                            ...
                            stage('SonarQube Scan') {
                                ...
                                if (env.SONAR_CREDENTIAL_ID != null && env.SONAR_CREDENTIAL_ID != '') {
                                    ...
                                    withSonarQubeEnv('pg-sonar') {
                                        sh ''' docker run -v "$WORKSPACE":/app -w /app sonarsource/sonar-scanner-cli:11.0 -Dsonar.python.coverage.reportPaths=coverage.xml -Dsonar.python.xunit.reportPath=testreport.xml -Dsonar.projectKey="$sonar_project_key"  -Dsonar.sources=. -Dsonar.projectName="$sonar_project_key" -Dsonar.host.url="$sonar_host" -Dsonar.organization="$sonar_org" -Dsonar.login=$SONAR_AUTH_TOKEN -Dsonar.token=$SONAR_AUTH_TOKEN '''
                                    }
                                }
                            }
